...
const Home = () => {
  ...
  const chunkCourses = () => {
    ...
    for (let i = 0; i < courses.length; i += 4) {
      const chunk = courses.slice(i, i + 4);
      if (chunk.length < 4) {
        const remaining = 4 - chunk.length;
        chunk.push(...courses.slice(0, remaining));
      }
    ...
    <div className="body">
      ...
      <Container fluid>
        ...
        <Carousel
          interval={null}
          indicators={false}
          prevIcon={<IoIosArrowDropleftCircle className="custom-arrow-cards prev-arrow-cards" />}
          nextIcon={<IoIosArrowDroprightCircle className="custom-arrow-cards next-arrow-cards" />}
          className="mt-5"
          activeIndex={index}
          onSelect={handleSelect}
        >
          {courseChunks.map((courseChunk, idx) => (
            <Carousel.Item key={idx}>
              <Row className="justify-content-center gap-2 mt-1">
                {courseChunk.map((course) => (
                  <Col key={course.id} md={2} className="cards-col mb-3 d-flex justify-content-center">
                    <Card className="card border border-2 border-light mt-1" onClick={() => handleOpenModal(course)}>
